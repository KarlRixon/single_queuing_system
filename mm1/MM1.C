...
float area_idle, last_idle, area_three, last_three, total_custs_time,
      cust_time[Q_LIMIT+1] = {0}, time_le_15min;
...
void initialize(void)  /* Initialization function. */
{
    ...
    server_status   = IDLE;
    num_in_q        = 0;
    time_last_event = 0.0;//上一事件结束时间

    /* Initialize the statistical counters. */

    num_custs_delayed  = 0;     //目前到达顾客总数
    total_of_delays    = 0.0;   //总等待时间
    area_num_in_q      = 0.0;   //用于计算平均等待队列长度
    area_server_status = 0.0;   //服务利用率
    area_idle          = 0.0;   //空闲概率
    last_idle          = 0.0;   //当前空前状态的开始时间
    area_three         = 0.0;   //店内有三个顾客的时间
    last_three         = 0.0;   //队列有两个顾客的开始时间
    total_custs_time   = 0.0;   //顾客在店内总时长
    time_le_15min      = 0.0;   //统计在店内逗留时间大于或等于15min的顾客人数

        /* Initialize event list.  Since no customers are present, the departure
       (service completion) event is eliminated from consideration. */

        time_next_event[1] = sim_time + expon(mean_interarrival); //下一个到达时间
    time_next_event[2] = 1.0e+30;//下一个出队时间
}
...
void report(void)  /* Report generator function. */
{
    ...
    fprintf(outfile, "Three customers probability%6.3f\n\n",
            area_three / sim_time);
    fprintf(outfile, "At least one customer%12.3f\n\n",
            1 - area_idle / sim_time);
    fprintf(outfile, "Average number of customer%7.3f\n\n",
            total_custs_time / sim_time);
    fprintf(outfile, "Average time of customer%9.3f\n\n",
            total_custs_time / num_delays_required);
